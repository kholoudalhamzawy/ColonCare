//
//  EnterRowViewController.swift
//  colonCancer
//
//  Created by KH on 15/07/2023.
//

import UIKit

class EnterRowViewController: UIViewController {
...
//        isModalInPresentation = true //makes the scrollview undissmisable
    ...
    @objc func submitNumber(){
        if isFinishedTypingNumber{
            errorLbl.text = "you must enter a row Number!"
        } else {
            if let number = Int(ageBtn.titleLabel!.text!) {
              
                isFinishedTypingNumber = true
                UploadViewViewModel.model.row = number
                UploadViewViewModel.model.validateSequanceRequest()
                dismiss(animated: true, completion: nil)
